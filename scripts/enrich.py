...
Longueur : dense, jamais délayé — vise 300 à 380 mots. Utile et concret vaut mieux que
long, mais SOUS 300 mots Yoast classe la page « texte trop court » et Google la juge
maigre (mesuré le 10/09 : 243 et 250 mots sur deux fiches en ligne, l'une verte l'autre
rouge). Le plancher de 300 n'est donc pas une préférence de style, c'est un seuil d'outil.
...
def enrich_event(ev: dict, material: str, client: anthropic.Anthropic, model: str,
    ...
    if _court:
        # 120-200 → 180-220 le 09/09, puis 280-320 le 10/09 : un « petit événement » est
        # publié comme les autres (WP#2234 : 90 mots en ligne) et Yoast lui applique le
        # MÊME seuil de 300 mots qu'aux autres — un mode court sous ce seuil condamne ces
        # fiches au rouge quoi qu'on fasse ailleurs. Le mode court reste moins cher par
        # l'absence de recherche web et COURT_MAX_TOKENS ; c'est ça qu'il économise,
        # pas la longueur du texte.
        prompt += ("\n\n[MODE COURT — petit événement] BREF mais pas maigre (280-320 mots, 2-3 "
