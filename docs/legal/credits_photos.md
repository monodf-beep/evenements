> ⚠️ **Modèle à faire valider par un juriste — ne constitue pas un conseil juridique.**
> Ce document est une trame rédactionnelle. Tous les éléments entre crochets `[…]` doivent être
> complétés par l'éditeur, et l'ensemble revu par un professionnel du droit avant mise en ligne.

---

# 🇫🇷 Politique de crédits photos
...
- **Aucune image servie par un CDN de presse ou d'agrégateur.** Nous excluons par principe les
  images provenant des serveurs d'images de médias de presse et d'agrégateurs
  *(liste technique de domaines bloqués maintenue par l'éditeur)*. La veille presse sert à
  **détecter** des événements, jamais à en **reprendre les images**.
...
- Toute image sous licence libre est accompagnée de son **crédit** : auteur, source et licence
  (par ex. *« Photo : [Auteur] — Wikimedia Commons — CC BY-SA 4.0 »*), avec lien lorsque la licence
  l'exige.
- Les visuels officiels sont attribués à leur **source** (lieu, organisateur) quand celle-ci le
  demande ou le justifie.
- Le texte alternatif (`alt`) décrit l'image de façon factuelle (nom de l'événement, lieu, ville).
...
# 🇮🇹 Politica dei crediti fotografici
...
- Ogni immagine con licenza libera è accompagnata dal suo **credito**: autore, fonte e licenza
  (ad es. *«Foto: [Autore] — Wikimedia Commons — CC BY-SA 4.0»*), con collegamento quando la
  licenza lo richiede.
- I materiali ufficiali sono attribuiti alla loro **fonte** (luogo, organizzatore) quando
  quest'ultima lo richiede o lo giustifica.
- Il testo alternativo (`alt`) descrive l'immagine in modo fattuale (nome dell'evento, luogo,
  città).
