...
> **Note (session repo, 2026-07-29 soir)** : avant de traiter un id de ce document, croiser avec
> `scripts/audit_wp_ids_local_match.py` (lecture seule) — certains posts WordPress listés ici ont
> une ligne correspondante dans `events_raw` (base du pipeline Python), auquel cas ils sont
> réparables via `scripts.enrich` / `scripts.translate_events --retranslate` (voix + panel de
> personas + révision automatiques) plutôt que par réécriture PHP à la main. Cas confirmé : l'id
> WP 3964 (Bug 1 ci-dessous) correspond à l'id local 4191, déjà enrichi avec `nice.fr` comme
> source officielle — à vérifier avant toute réécriture, pas forcément « cassé ».
...
| 1922 | John Ford au Cinema Massimo de Turin : l'Amérique en dix films | john-ford-au-cinema-massimo-de-torino-lamerique-en-dix-films |
