...
| `url_image_portrait` / `url_image_wide` posées à tort | `images_wide` (lecture de la page officielle, cron 10h35) | `images_wide --drop-portrait` (existait depuis le 08/09) **et `--drop-wide`** ; les deux se combinent, effacent la colonne et re-poussent la fiche, qui reprend alors `url_image` | ✅ **fermé le 2026-09-21** — le `wide` n'avait AUCUN rouvreur : une moitié de rouvreur laissait le plan de salle d'un théâtre en grand visuel 16:9, et ces deux colonnes PRIMENT sur `url_image` pour la vignette et le héros (`publisher_as`). Trois fiches réparées le jour même, quinze au total dans le lot |
| `image_source='manual'` | back-office (formulaire « URL de l'image ») ou une session, à la main | **personne, et c'est le but** : aucun cron ne remplace une image choisie par un humain (`moisson_officielle` teste `_src != 'manual'`, `_RANG_IMAGE` la met au plus haut) | 🟡 terminal **volontaire** — le rouvreur est l'humain qui recolle une autre URL. À savoir avant de s'étonner qu'une fiche ne suive plus le pipeline visuel |
