...
**Nuancé le 2026-09-08 — quand il n'y a PAS de page.** Mesuré ce jour-là : parmi 108 fiches approuvées, à venir et incomplètes, une vingtaine viennent d'une newsletter **sans aucun lien d'article** (`url_source` = `gmail:<message_id>#<rang>`, `gmail_relink` n'a rien trouvé). Pour elles, la chaîne visuels n'a rien à résoudre, et il leur manque toutes l'image, souvent la ville ou le lieu. `scripts/completer_depuis_mail.py` (cron 08:48, juste après `dates_depuis_mail`) les sert **sans modèle**, et sans revenir sur le constat ci-dessus :

- l'image n'est jamais « la première du mail » : c'est celle du **bloc HTML de l'annonce** (`utils/mail_html.py` lit le mail comme un arbre, localise le titre de la fiche, remonte au plus petit ancêtre porteur d'image qui ne contient pas le titre d'une autre annonce du même mail). Bloc partagé → rien ; mise en page à plat → rien, sauf si l'attribut `alt` reprend le titre. Puis les filtres existants (`_est_traqueur`, `is_logo_image`, `_is_chrome`, `remote_dims` ≥ 400 px de petit côté, pas une forme de bandeau). `image_source='mail'` ;
- le lieu et la ville sont **confirmés, pas extraits** (`utils/mail_lieux.py`) : un lieu du registre (`utils.lieux.registre()`, lieux par défaut de `config/sources.txt`) nommé dans l'annonce → lieu + ville ; sinon une commune du périmètre nommée dans l'annonce, avec sa majuscule, hors noms de bâtiments (« église Saint-Pierre ») et mots-pièges (« Contes pour enfants », « la salle des fêtes » : 26 des 784 communes sont aussi des mots courants) → ville, et le lieu si nos propres fiches le connaissent dans cette ville. Deux communes → rien. L'« annonce » est bornée par les titres des autres fiches du même mail, tous statuts confondus. `venue_source='mail'` ;
- **`gmail_collect` garde désormais le HTML** (`html_du_message`, table `gmail_html`, une ligne par message — pas par fiche, le HTML pèse 10 à 30 fois le texte). Les fiches collectées avant le 08/09 n'ont que `mail_corps` (texte, tronqué à 6 000 caractères) : leur mail est relu une fois dans Gmail par son identifiant, comme le fait `dates_depuis_mail`, puis mémorisé.

Ce que le mail **ne dit pas** reste un silence : un lieu absent du registre et une commune absente de nos listes ne produisent rien, et la fiche reste dans la file. Ce qui la fait sortir un autre jour, c'est le registre qui grandit (`config/lieux_villes.json`, une note de savoir, une ligne de `sources.txt`), pas un nouveau passage sur la même matière.
...
- **URL** — aucun nettoyage des paramètres de traçage à la collecte : `gmail_collect` garde volontairement les liens d'articles même avec `?utm_…` (paramètre courant sur des liens légitimes), et la dédup se fait en aval par `dedupe.py`. Cette ligne présentait `utils/sources.strip_tracking` comme active — la fonction existait mais n'était appelée nulle part ; retirée le 04/09.
