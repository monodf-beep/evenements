...
**Le vrai design**, lu directement dans `ui_kits/agenda/kit.css` + `components.jsx` + `colors_and_type.css` (Claude Design) : la carte est une **ligne dense de liste** (`.ag-row`, grid `96px(heure) | 1fr(contenu) | auto(statut)`), **sans image, sans pastille de date par ligne** — la date vit dans un **en-tête de groupe par jour** (`.ag-daygroup`) au-dessus d'un paquet de lignes. Détail vérifié dans `wordpress/design-system/components.css` (corrigé) et le JSX source.
