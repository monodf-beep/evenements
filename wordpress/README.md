...
| `scripts/apply-annoncer-template.mjs` | Pousse le snippet PHP de la page "Annoncer" (page 995, brouillon — pas encore publiée), formulaire envoie un e-mail (`wp_mail`) |
...
| 10 | Annoncer (page B2B + formulaire) | 🟡 Gabarit prêt (formulaire envoie un e-mail via `wp_mail`, pas de stockage WP) mais page (995, "Annoncer") créée en **brouillon** — même contrainte que Le Fil, non vérifiable avant publication (404 en GET/POST tant que non publiée) |
| 11 | Contact (page 1699, e-mail + formulaire) | ✅ v1 : `contact-template.php`, même pattern nonce/honeypot/`wp_mail` qu'Annoncer, adresse `contact@culturasabauda.eu` en clair + formulaire nom/e-mail/message. Snippet `CS · Gabarit Contact` (id 49). Vérifié en live (page publiée), 200 OK, formulaire vide propre, aucune erreur PHP |
| 12 | Confidentialité (page 1701, RGPD) | 🟡 Gabarit prêt (`legal-confidentialite-template.php`, snippet `CS · Gabarit Confidentialité` id 51) — rendu FR+IT vérifié en profondeur côté serveur (rejoue `get_header`+contenu+`get_footer`, aucune erreur PHP, tags équilibrés) mais page (1701) laissée en **brouillon** : le document source contient des informations réelles manquantes ou non tranchées (raison sociale, adresse, DPO, hébergeur, outil d'analytics réellement utilisé, durées de conservation) qu'il est interdit d'inventer. Détail dans `build-recipes/STATUS.md` |
| — | Explorations.dc.html | ⛔ Hors périmètre — c'est un canvas interne Claude Design (`design_doc_mode: canvas`, exploration de variantes), pas une maquette de page du site |
