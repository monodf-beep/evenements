#!/usr/bin/env node
/**
 * apply-homepage-template.mjs — Pousse le snippet PHP qui prend le contrôle
 * du rendu de l'Accueil (928) via template_redirect, pour bypasser le
 * gabarit page.php de GeneratePress (entry-header + sidebar). Voir
 * wordpress/design-system/homepage-template.php.
 *
 * Usage : node wordpress/scripts/apply-homepage-template.mjs
 */
...
const NAME = 'CS · Gabarit Accueil (928)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'homepage-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
