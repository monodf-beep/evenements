#!/usr/bin/env node
/**
 * apply-liste-evenements-template.mjs — Pousse le snippet PHP qui prend le
 * contrôle du rendu de "Ce week-end" (930) et "Tout l'agenda" (932) via
 * template_redirect, avec la vraie carte "compacte" (cs_card_compact).
 * Remplace le Listing Grid JetEngine poussé par apply-liste-pages.mjs (le
 * contenu Gutenberg des pages devient inutilisé, template_redirect intercepte
 * avant — même schéma que apply-taxonomy-archive-template.mjs).
 *
 * Usage : node wordpress/scripts/apply-liste-evenements-template.mjs
 */
...
const NAME = 'CS · Gabarit Ce week-end / Tout l\'agenda';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'liste-evenements-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
