#!/usr/bin/env node
/**
 * apply-article-single-template.mjs — Pousse le snippet PHP de la fiche
 * article "Le Fil" (native `post`, is_single()). Voir
 * wordpress/design-system/article-single-template.php.
 *
 * Usage : node wordpress/scripts/apply-article-single-template.mjs
 */
...
const NAME = 'CS · Gabarit Article (Le Fil)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'article-single-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
