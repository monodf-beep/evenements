#!/usr/bin/env node
/**
 * apply-proposer-evenement-template.mjs — Pousse le snippet PHP qui prend le
 * contrôle du rendu ET du traitement de la page "Proposer un événement" (934).
 * Voir wordpress/design-system/proposer-evenement-template.php.
 *
 * Usage : node wordpress/scripts/apply-proposer-evenement-template.mjs
 */
...
const NAME = 'CS · Gabarit Proposer un événement';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'proposer-evenement-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
