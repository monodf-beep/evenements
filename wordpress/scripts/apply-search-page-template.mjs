#!/usr/bin/env node
/**
 * apply-search-page-template.mjs — Pousse le snippet PHP qui prend le
 * contrôle du rendu de la page de recherche (is_search(), template_redirect).
 * Voir wordpress/design-system/search-page-template.php.
 *
 * Usage : node wordpress/scripts/apply-search-page-template.mjs
 */
...
const NAME = 'CS · Gabarit Recherche';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'search-page-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
