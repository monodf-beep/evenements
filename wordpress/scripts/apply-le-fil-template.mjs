#!/usr/bin/env node
/**
 * apply-le-fil-template.mjs — Pousse le snippet PHP du listing "Le Fil"
 * (page 994, brouillon). Voir wordpress/design-system/le-fil-template.php.
 *
 * Usage : node wordpress/scripts/apply-le-fil-template.mjs
 */
...
const NAME = 'CS · Gabarit Le Fil (listing)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'le-fil-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
