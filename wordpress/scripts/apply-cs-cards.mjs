#!/usr/bin/env node
/**
 * apply-cs-cards.mjs — Pousse les composants carte partagés (cs_card_standard,
 * cs_card_compact, cs_card_rail, cs_pill_class, etc.) comme snippet PHP global.
 * Doit être actif pour que single-event-meta.php, taxonomy-archive-template.php
 * et apply-liste-events.mjs fonctionnent (ils appellent ces fonctions).
 *
 * Usage : node wordpress/scripts/apply-cs-cards.mjs
 */
...
const NAME = 'CS · Composants carte (partagé)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'cs-cards.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
  const list = await api(`${API}?_fields=id,name`);
  const existing = Array.isArray(list) ? list.find((s) => s.name === NAME) : null;
  const payload = { name: NAME, code: php, scope: 'global', active: true, priority: 1 };
  const snip = existing
