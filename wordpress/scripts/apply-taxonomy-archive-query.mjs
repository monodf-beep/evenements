#!/usr/bin/env node
/**
 * apply-taxonomy-archive-query.mjs — Pousse le snippet PHP qui force les archives
 * de taxonomie (territoire, tribe_events_cat) à requêter tribe_events au lieu de
 * post. Voir wordpress/design-system/taxonomy-archive-query.php.
 *
 * Usage : node wordpress/scripts/apply-taxonomy-archive-query.mjs
 */
...
const NAME = 'CS · Archives taxonomie (territoire/catégories)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'taxonomy-archive-query.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
