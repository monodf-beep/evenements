#!/usr/bin/env node
/**
 * apply-search-events.mjs — Pousse le snippet PHP qui élargit la recherche aux
 * événements (tribe_events) et ajoute date+territoire sous chaque résultat.
 * Voir wordpress/design-system/search-events.php.
 *
 * Usage : node wordpress/scripts/apply-search-events.mjs
 */
...
const NAME = 'CS · Recherche (inclut événements)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'search-events.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
