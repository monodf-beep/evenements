#!/usr/bin/env node
/**
 * apply-single-event-meta.mjs — Pousse wordpress/design-system/single-event-meta.php
 * comme snippet PHP (scope front-end) via Code Snippets. Ajoute les champs manquants
 * (pilule territoire, badge de statut, crédit photo, "Vérifié le") sur la fiche
 * événement native de The Events Calendar.
 *
 * Idempotent. Usage : node wordpress/scripts/apply-single-event-meta.mjs
 */
...
const NAME = 'CS · Fiche événement (meta)';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'single-event-meta.php'), 'utf8')
    .replace(/^<\?php\s*/, ''); // Code Snippets ajoute déjà les balises PHP
