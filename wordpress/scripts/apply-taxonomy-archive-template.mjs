#!/usr/bin/env node
/**
 * apply-taxonomy-archive-template.mjs — Pousse le snippet PHP qui prend le
 * contrôle du rendu des Hubs territoire/catégorie (template_redirect), pour
 * réutiliser le style .ag-row du design system. Voir
 * wordpress/design-system/taxonomy-archive-template.php.
 *
 * Usage : node wordpress/scripts/apply-taxonomy-archive-template.mjs
 */
...
const NAME = 'CS · Gabarit Hub territoire/catégorie';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'taxonomy-archive-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
