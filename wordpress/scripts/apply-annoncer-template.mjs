#!/usr/bin/env node
/**
 * apply-annoncer-template.mjs — Pousse le snippet PHP de la page "Annoncer"
 * (page 995, brouillon). Voir wordpress/design-system/annoncer-template.php.
 *
 * Usage : node wordpress/scripts/apply-annoncer-template.mjs
 */
...
const NAME = 'CS · Gabarit Annoncer';
...
async function main() {
  const php = readFileSync(join(REPO, 'wordpress', 'design-system', 'annoncer-template.php'), 'utf8')
    .replace(/^<\?php\s*/, '');
